...
|-------|-------------|------------|
| `sleeping` | vLLM process running but CUDA checkpoint active | **0 bytes** |
...
stateDiagram-v2
    ...
    deactivating --> sleeping: requests drained, CUDA checkpoint
    serving --> sleeping: sleep (no in-flight requests)
    pending --> waking: preemption frees memory
`} />

<Callout type="info" title="True zero VRAM">
When sleeping, GreenThread calls vLLM's `/sleep?level=2` endpoint which performs a full CUDA checkpoint. This releases **all** GPU memory — not just most of it. Sleeping models consume exactly 0 bytes of VRAM, making the GPU fully available for other models.
</Callout>
...
4. **CUDA checkpoint** — calls vLLM `/sleep?level=2` to checkpoint GPU state
